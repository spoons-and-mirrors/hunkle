...
use super::{
    ...
    ScheduledRun, ScheduledTask, opencode_session as latest_message,
    scheduler::LegacyScheduledRunBinding, settings::AgentTimeDisplay,
};
...
pub(crate) use client::{HerdrPaneLayout, SchedulerObserveResult};
#[cfg(test)]
pub(crate) use client::{HerdrPaneRect, SchedulerLaunchRequest, SchedulerLaunchResult};

const REFRESH_INTERVAL: Duration = Duration::from_secs(2);
const BACKGROUND_PROBE_ATTEMPTS: u8 = 3;
const BACKGROUND_RETRY_INTERVAL: Duration = Duration::from_secs(30);
const BACKGROUND_FAILURES_BEFORE_DETACH: u8 = 3;
...
pub(super) fn create_tab_with_agent(
    ...
    client::create_tab_with_agent(path, workspace_id, session_id)
}
...
pub(crate) struct HerdrSession {
    ...
    background_snapshot_failures: u8,
...
impl HerdrSession {
    pub(crate) fn detect(config_dir: Option<&Path>) -> Self {
        ...
        if let Some(environment) = environment {
            ...
            #[cfg(not(test))]
            {
                session.background_detection = true;
                session.background_probe_attempts = BACKGROUND_PROBE_ATTEMPTS;
            }
        }
        session
    }
    ...
    fn new(
        ...
        let agent_timings_path_for_persistence = agent_timings_path.clone();
        Self {
            enabled,
            background_attached: false,
            background_detection: false,
            background_probe_attempts: 0,
            background_snapshot_failures: 0,
            ...
            agent_timings_path,
            agent_timing_persistence: agent_timings_path_for_persistence
                .filter(|_| enabled)
    ...
    pub(crate) fn is_background_attached(&self) -> bool {
        self.background_attached
    }

    pub(crate) fn agent_stash_running(&self) -> bool {
        self.agent_stash_running
    }

    pub(crate) fn should_poll(&self) -> bool {
        self.enabled || self.background_detection || self.loading
    }

    pub(crate) fn background_workspace_id(&self) -> Option<&str> {
        self.background_attached
            .then_some(self.host_workspace_id.as_deref())
            .flatten()
    }
    ...
    pub(crate) fn observe_scheduled_run(
        pane_id: &str,
        terminal_id: Option<&str>,
    ) -> SchedulerObserveResult {
        client::scheduler_observe(pane_id, terminal_id)
    }
        ...
        while let Ok(completion) = self.receiver.try_recv() {
            ...
            match completion {
                Completion::Snapshot {
                    ...
                    match result {
                        Ok(snapshot) => {
                            self.background_snapshot_failures = 0;
                            self.snapshot_generation =
                                self.snapshot_generation.max(request_generation);
                            if self.enabled {
                                let attachment_lost = self.background_attached
                                ...
                                if attachment_lost {
                                        ...
                                        snapshot.focused_workspace_id.clone()
                                    {
                                        ...
                                        self.detach_background(observed_at_ms);
                                        poll.notice = Some(
                                            "Herdr workspace disappeared; continuing standalone"
                                                .to_owned(),
                                        );
                                    }
                                }
                            ...
                            } else if let Some(workspace_id) = snapshot.focused_workspace_id {
                                self.enabled = true;
                        ...
                        Err(error) => {
                            if self.background_attached {
                                self.background_snapshot_failures =
                                    self.background_snapshot_failures.saturating_add(1);
                                if self.background_snapshot_failures
                                    >= BACKGROUND_FAILURES_BEFORE_DETACH
                                {
                                    self.detach_background(observed_at_ms);
                                    poll.notice = Some(
                                        "Herdr disconnected; continuing standalone".to_owned(),
                                    );
                                } else {
    ...
    fn detach_background(&mut self, observed_at_ms: u64) {
        self.enabled = false;
        self.background_attached = false;
        self.background_detection = true;
        self.background_probe_attempts = BACKGROUND_PROBE_ATTEMPTS;
        self.background_snapshot_failures = 0;
        self.host_workspace_id = None;
        self.host_tab_id = None;
        self.host_pane_id = None;
        self.workspaces.clear();
        self.apply_agent_snapshot_at(Vec::new(), observed_at_ms);
        self.error = None;
        self.next_refresh = Instant::now() + BACKGROUND_RETRY_INTERVAL;
    }
    ...
    pub(crate) fn legacy_scheduled_run_bindings(
        &self,
        tasks: &[ScheduledTask],
        runs: &[ScheduledRun],
    ) -> Vec<LegacyScheduledRunBinding> {
        runs.iter()
            ...
            .filter_map(|run| {
                ...
                let agent = exact.or_else(|| {
                    let task = tasks.iter().find(|task| task.id == run.task_id)?;
                ...
                Some(LegacyScheduledRunBinding {
                    run_id: run.id,
                    agent: if run.terminal_id.is_none() {
                        agent
                            .terminal_id
                            .clone()
                            .map(|terminal_id| (agent.pane_id.clone(), terminal_id))
                    } else {
                        None
                    },
                    session_id: run.session_id.is_none().then_some(session_id).flatten(),
                })
            })
            .collect()
    }
    ...
    fn start_snapshot(&mut self) {
        ...
        let sender = self.sender.clone();
        let background_attached = self.background_attached;
        thread::spawn(move || {
            let result = if probing {
                client::probe_session_snapshot()
            } else if background_attached {
                client::background_session_snapshot()
            } else {
                client::session_snapshot()
            }
            .map(|mut snapshot| {
                populate_workspace_branches(&mut snapshot.workspaces);
                snapshot
            });
    ...
    pub(crate) fn ready_for_test(value: &Value) -> Self {
        ...
        let value = client::complete_snapshot_for_test(value);
    ...
    #[cfg(test)]
    pub(crate) fn set_background_attached_for_test(&mut self, workspace: &str) {
        self.background_attached = true;
        self.background_detection = false;
        self.background_probe_attempts = 0;
        self.host_workspace_id = Some(workspace.to_owned());
...
mod presentation_interest_tests {
    ...
    #[test]
    fn repeated_background_snapshot_failures_return_to_standalone_discovery() {
        let mut session = HerdrSession::new(false, None, None);
        session.enabled = true;
        session.background_attached = true;
        session.host_workspace_id = Some("w1".to_owned());
        session.workspaces = vec![test_workspace("w1")];
        session.agents = vec![working_agent(None)];
        session.observed_agents = session.agents.clone();
        for request_generation in 1..=u64::from(BACKGROUND_FAILURES_BEFORE_DETACH) {
            session
                .sender
                .send(Completion::Snapshot {
        ...
        assert_eq!(
            ...
            Some("Herdr disconnected; continuing standalone")
        );
    }

    #[test]
    fn vanished_background_workspace_without_a_new_focus_clears_stale_agents() {
        let mut session = HerdrSession::new(false, None, None);
        session.enabled = true;
        session.background_attached = true;
        session.host_workspace_id = Some("w1".to_owned());
    ...
    #[test]
    fn exhausted_background_probes_continue_at_a_slow_interval() {
        let mut session = HerdrSession::new(false, None, None);
        session.background_detection = true;
        session.loading = true;
        session
        ...
        assert!(session.should_poll());
        assert!(!session.is_enabled());
        assert!(session.next_refresh > Instant::now());
        assert_eq!(session.error, None);
    }

    #[test]
    fn background_snapshot_without_a_focused_workspace_retries_slowly() {
        let mut session = HerdrSession::new(false, None, None);
        session.background_detection = true;
        session.loading = true;
        session
...
mod latest_user_message_cache_tests {
    ...
    fn binds_a_scheduled_run_to_its_live_terminal_and_session() {
        let mut session = HerdrSession::new(true, None, None);
        let tasks = vec![scheduler::ScheduledTask {
            id: 7,
        ...
        let runs = vec![scheduler::ScheduledRun {
            id: 9,
            task_id: 7,
            created_at_ms: 1,
        ...
        let bindings = session.legacy_scheduled_run_bindings(&tasks, &runs);
        assert_eq!(bindings.len(), 1);
        assert_eq!(
            bindings[0].agent,
            Some(("w2:p9".to_owned(), "term-w2:p9".to_owned()))
        );
        assert_eq!(bindings[0].session_id.as_deref(), Some("ses_remote"));
    }
