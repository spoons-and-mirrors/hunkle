    ...
    fn start_remote_load(&self, root: &Path, kind: RemoteKind) {
        ...
        thread::spawn(move || {
            let result = match kind {
                RemoteKind::PullRequests => {
                    RemoteResult::PullRequests(github::load_pull_requests(&root))
                }
                RemoteKind::Issues => RemoteResult::Issues(github::load_issues(&root)),
            };
...
mod tests {
    ...
    #[test]
    fn filters_each_tab_and_opens_branches() {
