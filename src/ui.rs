...
fn draw_agent_pane_picker_overlay(frame: &mut Frame<'_>, app: &mut App) {
    if app.herdr_prompt.agent_pane_picker_open() {
        dim(frame);
        for (target, rect) in draw_agent_pane_picker(
            frame,
            &app.herdr_prompt,
...
fn draw_agent_pane_picker(
    ...
    fill(frame, Rect::new(area.x, area.y, area.width, 3), palette().surface_alt);
    fill(
        frame,
    ...
    for (index, pane) in layout.panes.iter().enumerate() {
        ...
        let pane_area = Rect::new(
            ...
            right.saturating_sub(left).min(scaled.width.saturating_sub(left)),
            bottom.saturating_sub(top).min(scaled.height.saturating_sub(top)),
        );
...
fn fit_pane_layout(area: Rect, source_width: u16, source_height: u16) -> Rect {
    ...
    let (width, height) = if width_limited {
        let height = (u32::from(source_height) * u32::from(area.width)
            / u32::from(source_width))
        .max(1) as u16;
        (area.width, height)
    } else {
        let width = (u32::from(source_width) * u32::from(area.height)
...
fn scale_pane_edge(value: u16, origin: u16, source: u16, target: u16) -> u16 {
    ...
    ((u32::from(relative) * u32::from(target) + u32::from(source) / 2)
        / u32::from(source)) as u16
}
...
fn draw_header_badge_border(frame: &mut Frame<'_>, rect: Rect, color: Color) {
...
fn draw_header_picker(frame: &mut Frame<'_>, app: &mut App) {
        ...
        if destination_title {
            ...
            frame.render_widget(
                Paragraph::new(" START AGENT")
                    .style(Style::default().fg(palette().green).add_modifier(Modifier::BOLD)),
    ...
    if app.header_picker.items.is_empty() {
        ...
        frame.render_widget(
            ...
            Rect::new(
                area.x,
                area.y.saturating_add(item_offset),
                area.width,
                1,
            ),
        );
...
fn header_picker_label_line(
    ...
    let label_width =
        width.saturating_sub(UnicodeWidthStr::width(prefix.as_str()) + stats_width);
    let mut spans = vec![Span::styled(prefix, label_style)];
    spans.push(Span::styled(truncate_width(label, label_width), label_style));
