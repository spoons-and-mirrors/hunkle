...
- In Explorer, click the path field to type, a row to select it, or a preview to continue completing. Drag the divider between AROUND HERE and CONTENTS to resize the panes. AROUND HERE shows the ancestor branch, current folder, and its child folders so you can traverse up or down entirely in the left pane. Double-click a folder or press `Enter` to make it the current location, updating PATH, AROUND HERE, and CONTENTS together, including when that folder is a Git repository. Double-click the parent row to go back. Use the `Open current repository` or `Open current location` row to open that directory as a workspace; files open on double-click or `Enter`.
