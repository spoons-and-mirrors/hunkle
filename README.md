...
In Explorer, **Around Here** shows ancestors and neighboring directories while **Contents** lists the directories and files inside the current location; `Tab` switches panes. Start typing a folder name, press `p` to search from an empty field, `~` to complete from your home directory, or `/` to start an absolute path. Search accepts fuzzy directory names, relative paths, absolute paths, and `~/...`; path matches include hidden directories and files alongside a live child preview. The path field supports cursor editing and `Ctrl+Backspace` or `Alt+Backspace` removes the previous path segment. `Tab` accepts the best completion, adding a trailing `/` for directories, and `Enter` opens a repository, navigates into a directory, or opens a file's parent workspace with that file selected. Hidden directories are browseable, `.config` participates in background search, and only Git metadata and expensive generated trees are omitted from indexing.
...
- In Explorer, click the path field to type, a row to select it, or a preview to continue completing. AROUND HERE shows the ancestor branch, current folder, and its child folders so you can traverse up or down entirely in the left pane. Double-click a folder or press `Enter` to make it the current location, updating PATH, AROUND HERE, and CONTENTS together, including when that folder is a Git repository. Double-click the parent row to go back. Use the `Open current repository` or `Open current location` row to open that directory as a workspace; files open on double-click or `Enter`.
